...
import { useItemsSparepartList, useDeleteItemsSparepart } from './Hooks/useItemsSparepartList';
import { encodeId } from '@/Utils/Helpers/IdHelper';

export default function ItemsSparepart() {
    ...
    const { data, isLoading } = useItemsSparepartList({ search, limit, page });

    // Mengambil data dan meta dari response API
    const rows = data?.data || [];
    const meta = data?.meta;

    const deleteMutation = useDeleteItemsSparepart();
    ...
    return (
        <PortalLayout>
            <div className="space-y-6">
                <div>
                    <h1 className="text-2xl font-semibold text-slate-800">Master Items Sparepart</h1>
                    <div className="flex items-center gap-2 mt-1 text-sm text-slate-400">
                        <span>Master Data</span>
                        <span>/</span>
                        <span className="text-blue-600 font-medium">Items Sparepart</span>
                    </div>
                </div>

                <div className="bg-white rounded-xl border border-slate-200 shadow-sm">
                    <div className="p-5 border-b border-slate-200 flex justify-between items-center">
                        <h2 className="text-lg font-semibold">Data Items Sparepart</h2>
                        <button
                            onClick={() => navigate('/portal/master/items-sparepart/create')}
                            ...
                            <table className="min-w-full text-sm">
                                <thead className="bg-slate-100">
                                    <tr>
                                        ...
                                        <th className="px-4 py-3 text-left">Part Code</th>
                                    ...
                                    {isLoading ? (
                                        ...
                                        rows.map((item, index) => (
                                            <tr key={item.id} className="border-t">
                                                ...
                                                <td className="px-4 py-3 font-medium">{item.part_code}</td>
                                                <td className="px-4 py-3 font-medium">{item.part_name}</td>
                                                ...
                                                <td className="px-4 py-3">
                                                    <div className="flex justify-center gap-3">
                                                        <button onClick={() => navigate(`/portal/master/items-sparepart/edit/${encodeId(item.id)}`)} className="text-amber-600 hover:text-amber-800"><Pencil size={18} /></button>
