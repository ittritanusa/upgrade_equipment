...
import { useTireTypeKendaraanList, useDeleteTireTypeKendaraan } from './Hooks/useTireTypeKendaraanList';
import { encodeId } from '@/Utils/Helpers/IdHelper';

export default function TireTypeKendaraan() {
    ...
    const { data, isLoading } = useTireTypeKendaraanList({ search, limit, page });

    // Mengambil data dan meta dari response API
    const rows = data?.data || [];
    const meta = data?.meta;

    const deleteMutation = useDeleteTireTypeKendaraan();
    ...
    return (
        <PortalLayout>
            <div className="space-y-6">
                <div>
                    <h1 className="text-2xl font-semibold text-slate-800">Master Tire Type Kendaraan</h1>
                    <div className="flex items-center gap-2 mt-1 text-sm text-slate-400">
                        <span>Master Data</span>
                        <span>/</span>
                        <span className="text-blue-600 font-medium">Tire Type Kendaraan</span>
                    </div>
                </div>

                <div className="bg-white rounded-xl border border-slate-200 shadow-sm">
                    <div className="p-5 border-b border-slate-200 flex justify-between items-center">
                        <h2 className="text-lg font-semibold">Data Tire Type Kendaraan</h2>
                        <button
                            onClick={() => navigate('/portal/master/tire-type/create')}
                            ...
                            <table className="min-w-full text-sm">
                                <thead className="bg-slate-100">
                                    <tr>
                                        ...
                                        <th className="px-4 py-3 text-left">Kode Type</th>
                                        <th className="px-4 py-3 text-left">Jumlah Tire</th>
                                    ...
                                    {isLoading ? (
                                        ...
                                        rows.map((item, index) => (
                                            <tr key={item.id} className="border-t">
                                                ...
                                                <td className="px-4 py-3 font-medium">{item.KodeType}</td>
                                                <td className="px-4 py-3">{item.Tire}</td>
                                                ...
                                                <td className="px-4 py-3">
                                                    <div className="flex justify-center gap-3">
                                                        <button onClick={() => navigate(`/portal/master/tire-type/edit/${encodeId(item.id)}`)} className="text-amber-600 hover:text-amber-800"><Pencil size={18} /></button>
