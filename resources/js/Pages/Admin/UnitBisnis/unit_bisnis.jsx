...
import { useUnitBisnisList, useDeleteUnitBisnis } from './Hooks/useUnitBisnisList';
import { encodeId } from '@/Utils/Helpers/IdHelper';

export default function UnitBisnis() {
    ...
    const { data, isLoading } = useUnitBisnisList({ search, limit, page });

    // Mengambil data dan meta dari response API
    const rows = data?.data || [];
    const meta = data?.meta;

    const deleteMutation = useDeleteUnitBisnis();
            ...
            <div className="space-y-6">
                ...
                <div className="bg-white rounded-xl border border-slate-200 shadow-sm">
                    <div className="p-5 border-b border-slate-200 flex justify-between items-center">
                        ...
                        <button
                            onClick={() => navigate('/portal/master/unit-bisnis/create')}
                            ...
                            <table className="min-w-full text-sm">
                                <thead className="bg-slate-100">
                                    <tr>
                                        ...
                                        <th className="px-4 py-3 text-left">Kode Unit Bisnis</th>
                                    ...
                                    {isLoading ? (
                                        ...
                                        rows.map((item, index) => (
                                            <tr key={item.id} className="border-t">
                                                ...
                                                <td className="px-4 py-3 font-medium">{item.KodeUnitBisnis}</td>
                                                ...
                                                <td className="px-4 py-3">
                                                    <div className="flex justify-center gap-3">
                                                        <button onClick={() => navigate(`/portal/master/unit-bisnis/edit/${encodeId(item.id)}`)} className="text-amber-600 hover:text-amber-800"><Pencil size={18} /></button>
