...
export default function UnitKendaraan() {
    ...
    const { data, isLoading } = useTipeKendaraanList({ search, limit });
    const rawRows = Array.isArray(data?.data?.data) ? data.data.data : [];

    const rows = rawRows.filter((item) => {
        const searchLower = search.toLowerCase();
        return (
            item.KodeUnit?.toLowerCase().includes(searchLower) ||
            item.KodeType?.toLowerCase().includes(searchLower) ||
                                            ...
                                            <tr key={item.id} className="border-t">
                                                ...
                                                <td className="px-4 py-3">
                                                    <div className="flex justify-center gap-3">
                                                        <button
                                                            onClick={() => navigate(`/portal/master/tipe-kendaraan/edit/${encodeId(item.id)}`)}
