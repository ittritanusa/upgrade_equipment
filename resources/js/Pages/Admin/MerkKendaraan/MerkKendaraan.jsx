...
import { useMerkKendaraanList, useDeleteMerkKendaraan } from './Hooks/useMerkKendaraanList';
import { encodeId } from '@/Utils/Helpers/IdHelper';

export default function MerkKendaraan() {
    const navigate = useNavigate();
    const [search, setSearch] = useState('');
    const [limit, setLimit] = useState(10);
    const [page, setPage] = useState(1);

    const { data, isLoading } = useMerkKendaraanList({ search, limit });
    ...
    const deleteMutation = useDeleteMerkKendaraan();
    ...
    return (
        <PortalLayout>
            <div className="space-y-6">
                {/* Header Section */}
                <div>
                    <h1 className="text-2xl font-semibold text-slate-800">Master Merk Kendaraan</h1>
                    <div className="flex items-center gap-2 mt-1 text-sm text-slate-400">
                        <span>Master Data</span>
                        <span>/</span>
                        <span className="text-blue-600 font-medium">Merk Kendaraan</span>
                    </div>
                </div>
                ...
                <div className="bg-white rounded-xl border border-slate-200 shadow-sm">
                    ...
                    <div className="p-5 border-b border-slate-200 flex justify-between items-center">
                        <h2 className="text-lg font-semibold">Data Merk Kendaraan</h2>
                        <button
                            onClick={() => navigate('/portal/master/merk-kendaraan/create')}
                            ...
                            <table className="min-w-full text-sm">
                                <thead className="bg-slate-100">
                                    <tr>
                                        ...
                                        <th className="px-4 py-3 text-left">Kode Merk</th>
                                        <th className="px-4 py-3 text-left">Merk Kendaraan</th>
                                    ...
                                    {isLoading ? (
                                        ...
                                        rows.map((item, index) => (
                                            <tr key={item.id} className="border-t">
                                                ...
                                                <td className="px-4 py-3 font-medium">{item.KodeMerk}</td>
                                                ...
                                                <td className="px-4 py-3">
                                                    <div className="flex justify-center gap-3">
                                                        <button
                                                            onClick={() => navigate(`/portal/master/merk-kendaraan/edit/${encodeId(item.id)}`)}
