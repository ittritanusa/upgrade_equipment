...
import { useCategorySparepartList, useDeleteCategorySparepart } from './Hooks/useCategorySparepartList';
import { encodeId } from '@/Utils/Helpers/IdHelper';

export default function CategorySparepart() {
    const navigate = useNavigate();
    const [search, setSearch] = useState('');
    const [limit, setLimit] = useState(10);
    const [page, setPage] = useState(1);

    const { data, isLoading } = useCategorySparepartList({ search, limit, page });

    const rows = data?.data || [];
    const meta = data?.meta;

    const deleteMutation = useDeleteCategorySparepart();
    ...
    return (
        <PortalLayout>
            <div className="space-y-6">
                <div>
                    <h1 className="text-2xl font-semibold text-slate-800">Master Kategori Sparepart</h1>
                    <div className="flex items-center gap-2 mt-1 text-sm text-slate-400">
                        <span>Master Data</span>
                        <span>/</span>
                        <span className="text-blue-600 font-medium">Kategori Sparepart</span>
                    </div>
                </div>

                <div className="bg-white rounded-xl border border-slate-200 shadow-sm">
                    <div className="p-5 border-b border-slate-200 flex justify-between items-center">
                        <h2 className="text-lg font-semibold">Data Kategori Sparepart</h2>
                        <button
                            onClick={() => navigate('/portal/master/category-sparepart/create')}
                            ...
                            <table className="min-w-full text-sm">
                                <thead className="bg-slate-50 text-slate-600 border-b border-slate-200">
                                    <tr>
                                        ...
                                        <th className="px-4 py-3 text-left font-semibold">Kode Kategori</th>
                                    ...
                                    {isLoading ? (
                                        ...
                                        rows.map((item, index) => (
                                            <tr key={item.id} className="hover:bg-slate-50/50 transition">
                                                ...
                                                <td className="px-4 py-3 font-medium">{item.category_code}</td>
                                                <td className="px-4 py-3 font-medium">{item.category_name}</td>
                                                <td className="px-4 py-3 font-medium">{item.description}</td>
                                                <td className="px-4 py-3">
                                                    <div className="flex justify-center items-center gap-2">
                                                        <button onClick={() => navigate(`/portal/master/category-sparepart/edit/${encodeId(item.id)}`)} className="p-1.5 text-amber-600 hover:bg-amber-50 rounded-lg transition"><Pencil size={18} /></button>
